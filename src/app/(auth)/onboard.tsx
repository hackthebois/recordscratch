...
import type { Onboard } from "@/types";
import { OnboardSchema, invalidHandleRegex } from "@/types";
...
const OnboardPage = () => {
	...
	const { mutateAsync: getSignedURL } = useMutation(
		api.profiles.getSignedURL.mutationOptions(),
	);
	...
	const handlePristine = useStore(
		form.store,
		(state) => state.fieldMeta.handle?.isPristine,
	);
	...
	const handleExists = useMutation(
		api.profiles.handleExists.mutationOptions(),
	);
